...
typedef struct {
	...
	int wheel_delta;			 // recent change in mouse wheel positive means up, negative means down
} cvui_mouse_t;
...
void handleMouse(int theEvent, int theX, int theY, int theFlags, void* theData) {
	...
	if (theEvent == cv::EVENT_MOUSEWHEEL) {
		aContext->mouse.wheel_delta = cv::getMouseWheelDelta(theFlags);
	}
	else {
		aContext->mouse.wheel_delta = 0;
	}
